...
現在の合宿デモでは、このスキーマを簡略化した `presence` 単一テーブルのみを
使用している（`docs/decisions/0002-demo-shortcuts.md` 参照）。
本開発では、以下の正式なスキーマに移行する。
...
2. 名前表示のロジックは、必ず `FRIEND_AREA_LINKS.status = 'approved'`
   ...
   誤って見えてしまう）
3. 論理削除（`deleted`フラグ）は使わない。位置情報・友達関係などの
   機微データは物理削除する（PRDのプライバシー方針に基づく）
